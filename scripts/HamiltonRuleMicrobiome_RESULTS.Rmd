...
# LOAD ANALYSIS OUTPUT (RData object)
load('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_1.RData')
...
- **Model 3**: *mean relatedness ~ 1  + sporulation score + mean relative abundance*. Two ecological factor classically predicted to shape relatedness (infinite island model background). Asking if these predictions hold in the case of microbiome. 
&nbsp;
...
- **Model 5**: *mean relatedness ~ cooperation + sporulation score + mean relative abundance*. This models completes the path analysis, asking if cooperative traits and ecological factor predict relatedness. 
&nbsp;
...
mod3.summary.formatted<- format.summary(m3.mean) %>%
  mutate_if(is.numeric, funs(formatC(., digit = 3, format = 'f')))
...
mods5.summary.formatted<- 
rbind(
format.summary(m.5.ss),
format.summary(m.5.biofilm),
format.summary(m.5.siderophores),
format.summary(m.5.ab),
format.summary(m.5.QS),
format.summary(m.5.SSyst)) %>%
  mutate_if(is.numeric, funs(formatC(., digit = 3, format = 'f')))


mod5.pmcmcs<- 
rbind(
get.pmcmc.from.summary(mods.5$ab_degradation, c('ab_degradation', 'mean_relative_abundance', 'sporulation_score')),
get.pmcmc.from.summary(mods.5$biofilm, c('biofilm', 'mean_relative_abundance', 'sporulation_score')),
get.pmcmc.from.summary(mods.5$siderophores, c('siderophores', 'mean_relative_abundance', 'sporulation_score')),
get.pmcmc.from.summary(mods.5$quorum_sensing, c('quorum_sensing', 'mean_relative_abundance', 'sporulation_score')),
get.pmcmc.from.summary(mods.5$secretion_system_no4, c('secretion_system', 'mean_relative_abundance', 'sporulation_score')),
get.pmcmc.from.summary(mods.5$secretome, c('nb_extracellular', 'mean_relative_abundance', 'sporulation_score'))
) %>%
  as.data.frame() %>%
  rename(cooperation = V1,
         mean_relative_abundance = V2,
         sporulation_score = V3) %>%
  mutate(`Cooperative trait` = c('ab_degradation', 'biofilm', 'siderophores', 'quorum_sensing','secretion_system', 'secretome')) %>%
  gather('predictor_variable', 'pMCMC', 1:3) %>%
  mutate(trait.predictor = paste0(`Cooperative trait`, '.', predictor_variable)) %>%
  select(trait.predictor, pMCMC)


dat.mod5.formatted<-
  dat.mod5 %>%
  mutate(trait.predictor = paste0(cooperative_trait, '.', predictor_variable)) %>%
  left_join(mod5.pmcmcs, by = 'trait.predictor') %>%
  select(predictor_variable, cooperative_trait, effect, sd_post, hpd_lower, hpd_higher, pMCMC) %>%
  rename(`Predictor` = predictor_variable,
         `Cooperative trait` = cooperative_trait,
         `Posterior mean` = effect,
         `Posterior sd` = sd_post,
         `HMP lower` = hpd_lower,
         `HMP higher` = hpd_higher) %>%
  mutate_if(is.numeric, funs(formatC(., digit = 3, format = 'f'))) %>%
  arrange(`Cooperative trait`, Predictor)
  


# Format a table of Meta-analysis results for display
MA.MODELS_5.formatted<- 
MA.MODELS_5 %>%
  as.data.frame() %>%
  rename(`Predictor` = predictor,
         `Estimate` = estimate,
...
```{r, echo = FALSE}
kable(dat.mod5.formatted, caption = "Cooperation, abundance and sporulation drivers of relatedness models summary", row.names = FALSE)
```


```{r, echo = FALSE}
kable(MA.MODELS_5.formatted, caption = "Cooperation, abundance and sporulation drivers of relatedness models Meta-analysis summary", row.names = FALSE)
```
...
# GR-test on model 1
mods.R.chain1 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_1.RData')[[c('mods.R')]]
mods.R.chain2 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_2.RData')[['mods.R']]
#mods.R.chain3 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_3.RData')[['mods.R']]
...
# GR-test on model 3
model3.chain1<- list(model_3 = loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_1.RData')[[c('m3.mean')]])
model3.chain2<- list(model_3 = loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_2.RData')[[c('m3.mean')]])
#model3.chain3<- list(model_3 = loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_3.RData')[[c('m3.mean')]])
...
# GR-test on model 4
mods.R.RA.SPO.chain1 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_1.RData')[[c('mods.R.RA.SPO')]]
mods.R.RA.SPO.chain2 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_2.RData')[['mods.R.RA.SPO']]
#mods.R.RA.SPO.chain3 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_3.RData')[['mods.R.RA.SPO']]
...
# GR-test on model 5
mods.5.chain1 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_1.RData')[[c('mods.5')]]
mods.5.chain2 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_2.RData')[['mods.5']]
#mods.5.chain3 <- loadToEnv('~/Documents/GitHub/HamiltonRuleMicrobiome/output/MODELS_CHAIN_3.RData')[['mods.5']]

mods.5.chains<- list(mods.5.chain1, mods.5.chain2)

gr.test.model5<- 
rbind(
run_gelman_rubin_test(mods.5.chains, 'secretome'),
run_gelman_rubin_test(mods.5.chains, 'ab_degradation'),
run_gelman_rubin_test(mods.5.chains, 'biofilm'),
run_gelman_rubin_test(mods.5.chains, 'secretion_system_no4'),
run_gelman_rubin_test(mods.5.chains, 'siderophores'),
run_gelman_rubin_test(mods.5.chains, 'quorum_sensing')) %>%
  mutate(model = 'model 5')


rm(list = c("mods.5.chain1",
            "mods.5.chain2",
            #"mods.5.chain3",
            "mods.5.chains"))
